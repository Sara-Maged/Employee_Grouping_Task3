...
public class Task3_App {
	...
	public static void main(String[] args) {
		...
		Map<String, Long> personByTitleC = new HashMap<>(personByTitleC = person
				.stream()
				.collect(Collectors.groupingBy(Employee::getEmployeeTitle, Collectors.counting())));
		
		personByTitle.forEach((k,v)->System.out.printf(
				"Title %s Count %s %s\n" ,
				k ,personByTitleC.get(k) ,
				v.toString().substring(1).replace(']', ' ').replace(',', ' ')));
	}
